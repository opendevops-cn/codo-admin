...
def _get_value(value: str = None):
    ...
    return or_(
        Menus.id == value,
        Menus.details.like(f'%{value}%'),
        Menus.menu_name.like(f'%{value}%')
    )
...
def get_menu_list_for_api(**params) -> dict:
    ...
    with DBContext('r') as session:
        page = paginate(
            session.query(Menus).filter(_get_value(value), _get_by_app_code(app_code)).filter_by(**filter_map),
            **params)
...
def get_menu_list_for_role(role_id: int) -> list:
    with DBContext('r') as db:
        role_func = db.query(Menus).outerjoin(RoleMenus, Menus.id == RoleMenus.menu_id
                                              ).filter(RoleMenus.role_id == role_id).all()
